...
-- 영어, 숫자, 특수기호는 1바이트 할당
SELECT
       BIT_LENGTH('한글') -- 6바이트, 48비트
     , CHAR_LENGTH('한글')
     , LENGTH('한글'); -- length는 바이트 길이
...
-- 10자리 문자열에서 '오른쪽' 삽입 후 나머지는 #로 채운다
SELECT LPAD('왼쪽', 10, '#'), RPAD('오른쪽', 10, '#');

-- 공백 제거
-- LTRIM(문자열), RTRIM(문자열)
-- TRIM(문자열), TRIM(방향 자를_문자열 FROM 문자열) / 양쪽 공백을 제거
